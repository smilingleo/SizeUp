...
@Test func skipListPreventsPlacement() {
    ...
    makeRouter(window: w, skipList: ["com.apple.QuickTimePlayerX"]).perform(.half(.left))
    #expect(w.applied.isEmpty)
    #expect(w.stored == CGRect(x: 5, y: 5, width: 300, height: 200))
}

@MainActor
@Test func noFocusedWindowIsHandledSilently() {
    makeRouter(window: nil).perform(.half(.left))
