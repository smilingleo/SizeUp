...
**Tech Stack:** Swift 6.3, swift-tools-version 6.3, Swift language mode v6, Swift Testing (`import Testing`), AppKit, ApplicationServices (Accessibility), Carbon.HIToolbox (hotkeys). No external package dependencies.
...
- No external SwiftPM dependencies. Everything uses the system SDK.
...
Run: `swift test --filter WindowKitTests`
Expected: FAIL — `WindowHandle` not found (until Step 1's file compiles, which it should) or the test target cannot see `Geometry`. If `Geometry` is not visible, add it to the `WindowKitTests` dependencies in `Package.swift`:

```swift
.testTarget(name: "WindowKitTests", dependencies: ["WindowKit", "Geometry"]),
```
...
```swift
.testTarget(name: "AppTests", dependencies: ["App", "Geometry", "WindowKit"]),
```
