...
**Two defects were found in an earlier version of this reference code by
fix-round-1 review and are corrected below. Read the callouts before
"simplifying" this back to the shorter, buggy shape — both shapes below are
deliberate, not accidental complexity.**

**Defect 1 — dangling `userData`, no teardown guarantee.** If a
`HotkeyManager` is ever deallocated without an explicit `unregisterAll()`
having run first, and the Carbon `EventHandlerRef`/`userData` pointer lives
directly on `HotkeyManager`, the installed handler and its `userData`
pointer outlive the object. The next hotkey press then dereferences freed
memory — a real use-after-free, not merely a race — and the hotkey stays
claimed system-wide until logout.

`HotkeyManager` is `@MainActor`, so it cannot fix this with a `deinit` that
calls its own `unregisterAll()`: that method is actor-isolated, and Swift 6
rejects a synchronous call into actor-isolated code from a nonisolated
`deinit`. Confirmed by actually trying it — the compiler says:

```
error: call to main actor-isolated instance method 'unregisterAll()' in a synchronous nonisolated context [#ActorIsolatedCall]
```

The fix is to move the Carbon handles into a separate, deliberately
non-isolated `CarbonHandles` class that `HotkeyManager` holds by strong
reference. `CarbonHandles.deinit` runs the same cleanup that
`unregisterAll()` calls, so ARC guarantees teardown the moment the last
strong reference goes away — whether that happens via an explicit
`unregisterAll()` or via ordinary deallocation of the owning
`HotkeyManager`. `UnregisterEventHotKey`/`RemoveEventHandler` are documented
as callable from any thread, so no actor isolation is needed on
`CarbonHandles`. The Carbon `userData` pointer is also changed to point at
`CarbonHandles` rather than at `HotkeyManager`, so its lifetime is tied
directly to the registrations it owns instead of to a separately-isolated
object that might outlive or be outlived by it.

**Defect 2 — `MainActor.assumeIsolated` can trap.** The callback wrapped its
body in `MainActor.assumeIsolated`, justified only by "Carbon delivers these
events on the main run loop." That is a long-standing convention for
`InstallEventHandler` on the application event target, not a documented API
guarantee. `assumeIsolated` does not degrade if the assumption is ever
false — it traps and crashes the app. The fix is an unconditional hop via
`DispatchQueue.main.async`, correct regardless of which thread the callback
arrives on, at the cost of one imperceptible run-loop turnaround before the
window moves. The callback still returns `noErr` immediately, before the
hop completes.
...
Also add, in the same test file, a test proving teardown happens without an
explicit `unregisterAll()` call — this is the whole point of Defect 1's
fix, so do not skip it:
