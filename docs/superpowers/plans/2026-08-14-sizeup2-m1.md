...
- **Repeat detection** is built into `WindowStateStore` now, because Snap Back needs the same "has the user moved this window?" test. The fractions array is `[0.5]` in M1, making behavior idempotent. M3 changes one array literal.
...
1. **`targetFrame` signature.** The spec wrote `targetFrame(for:on:gaps:current:cycleStep:)`. `cycleStep` is replaced by `fraction: CGFloat`, resolved by the caller from the fractions array. Rationale: it keeps `Geometry` ignorant of preferences and makes each fraction directly testable. `current` is retained — `.center` needs the window's existing size, since SizeUp's Center preserves size (confirmed: the `Center` entry in the user's plist has no `Width`/`Height` keys, unlike `Left`/`Right`/etc.).
...
**Files:**
- Create: `Package.swift`, `Sources/Geometry/Gaps.swift`, `Sources/Geometry/ScreenInfo.swift`, `Sources/Geometry/Action.swift`, `Sources/Geometry/FrameMath.swift`, `.gitignore` (already exists — verify), `LICENSE`
- Test: `Tests/GeometryTests/HalvesTests.swift`

**Interfaces:**
- Consumes: nothing.
- Produces: `Gaps(inner:outer:)`, `Gaps.zero`, `ScreenInfo(id:frame:visibleFrame:)`, `enum Edge { case left, right, top, bottom }`, `enum Corner { case upperLeft, upperRight, lowerLeft, lowerRight }`, `enum Direction { case next, previous, above, below }`, `enum Action`, and `targetFrame(for:on:gaps:current:fraction:) -> CGRect?`.
...
**Interfaces:**
- Consumes: `targetFrame(for:on:gaps:current:fraction:)`, `ScreenInfo`, `Gaps`, `Action`, `Corner` from Task 1.
...
Run: `swift test --filter GeometryTests`
Expected: PASS, 19 tests total. If `centerClampsOversizedWindow` or `fourQuartersTileTheScreenExactly` fails, fix `FrameMath.swift` — the tests define the contract.
...
public final class WindowStateStore {
    private struct State {
        ...
        /// the length of its fraction list.
    ...
    /// Which position in the fraction list the next placement should use.
    ///
...
Run: `swift test --filter WindowStateStoreTests`
Expected: PASS, 10 tests. `cycleStep` returns `step + 1`; the router wraps it against the fraction list length, which is verified in Task 8.
...
**Interfaces:**
- Consumes: `targetFrame(for:on:gaps:current:fraction:)`, `ScreenInfo`, `Gaps`, `Action` (Task 1); `WindowProviding`, `ScreenProviding`, `WindowHandle`, `WindowKey` (Task 4); `WindowStateStore` (Task 7).
- Produces: `@MainActor struct ActionRouter` with `init(screens:windows:store:gaps:fractions:skipList:)` and `func perform(_ action: Action)`.
...
/// With a single fraction, repeated presses are idempotent — M1 behavior.
@MainActor
@Test func repeatedPressIsIdempotentWithOneFraction() {
    let w = TestWindow(frame: CGRect(x: 0, y: 0, width: 800, height: 600))
    let router = makeRouter(window: w, fractions: [0.5])
    router.perform(.half(.left))
...
/// With three fractions, repeated presses advance and then wrap — M3 behavior,
/// verified now because the wrapping logic ships in this task.
@MainActor
@Test func repeatedPressCyclesThroughFractionsAndWraps() {
    let w = TestWindow(frame: CGRect(x: 0, y: 0, width: 800, height: 600))
    let router = makeRouter(window: w, fractions: [0.5, 2.0 / 3.0, 1.0 / 3.0])
    router.perform(.half(.left))
...
@Test func clampedWindowStillCyclesOnRepeat() {
    ...
    let router = makeRouter(window: w, fractions: [0.5, 1.0 / 3.0])
    router.perform(.half(.left))
...
public struct ActionRouter {
    ...
    public init(
        ...
        self.store = store
        self.gaps = gaps
        self.fractions = fractions.isEmpty ? [0.5] : fractions
    ...
    public func perform(_ action: Action) {
        ...
        default:
            ...
            let fraction = fractions[step % fractions.count]
            guard let target = targetFrame(
                for: action, on: screen, gaps: gaps, current: current, fraction: fraction
            ) else { return }
...
If `repeatedPressCyclesThroughFractionsAndWraps` fails at the wrap, check that `WindowStateStore.record` stores the advanced step only when the action repeats — the store's `step` must increase by exactly one per repeat, and the router applies `% fractions.count`.

- [ ] **Step 5: Run the whole suite and commit**

Run: `swift test`
Expected: PASS, 59 tests across four test targets (Geometry 19, WindowKit 7, Hotkeys 11, App 22).
...
| 2 | Press ⌃⌥⌘← again | Nothing changes (single fraction in M1) |
...
- `swift test` passes: 61 tests across four test targets (Geometry 19, WindowKit 7, Hotkeys 11, App 24).
